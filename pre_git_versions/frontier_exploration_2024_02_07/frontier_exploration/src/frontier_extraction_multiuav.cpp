...
public:
    ...
    void octomapCallback1(const octomap_msgs::Octomap::ConstPtr& msg)
    {
        ...
        if(mergedOctree)
            octree1 = mergeOctrees(octree1, mergedOctree);
    }
    ...
    void octomapCallback2(const octomap_msgs::Octomap::ConstPtr& msg)
    {
        ...
        if(mergedOctree)
            octree2 = mergeOctrees(octree2, mergedOctree);
    }
    ...
    std::unique_ptr<octomap::OcTree> mergeOctrees(const std::unique_ptr<octomap::OcTree>& o1, const std::unique_ptr<octomap::OcTree>& o2)
    {
        std::unique_ptr<octomap::OcTree> mergedOctree = std::make_unique<octomap::OcTree>(*o1);

        for(octomap::OcTree::leaf_iterator it = o2->begin_leafs(), end = o2->end_leafs(); it != end; ++it)
        {
            mergedOctree->updateNode(it.getKey(), it->getValue());
        }
        ...
        while (ros::ok())
        {
            ...
            switch (currentState)
            {
                ...
                case PROXIMITY:
                    ...
                    if(octree2) calculateCoverage(octree2);
                    if(mergedOctree) calculateCoverage(mergedOctree);
                ...
                case COOP:
                    ...
                    publishBestFrontier(mergedOctree, bestFrontiers[0], "bestFrontier1", 1.0, 1.0, 0.0);
                    publishBestFrontier(mergedOctree, bestFrontiers[1], "bestFrontier2", 1.0, 0.0, 1.0);
