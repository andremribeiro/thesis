...
public:
    ...
    void odomCallback(const nav_msgs::OdometryConstPtr& msg)
    {
        ...
        // if(octree1)
        // {
        //     octomap::OcTreeKey currentKey = octree1->coordToKey(pos);
    ...
    void calculateCoverage(const std::unique_ptr<octomap::OcTree>& octree)
    {
        ...
        octomap::point3d min(explorationMinX, explorationMinY, explorationMinZ);
        ...
        totalCells = ((explorationMaxX - explorationMinX) * (explorationMaxY - explorationMinY) * (explorationMaxZ - explorationMinZ)) / std::pow(resolution, 3);  // Total number of cells in the area
    ...
    octomap::KeySet frontierClustering(const std::unique_ptr<octomap::OcTree>& octree, octomap::KeySet& frontierCells)
    {
		...
		cluster->getMeanShiftClusters(originalPointsVector, clusteredPointsVector, 1.0);
