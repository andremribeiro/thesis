...
public:
    ...
    void frontierEvaluation()
    {
        ...
        for(octomap::KeySet::iterator iter = clusters.begin(), end = clusters.end(); iter != end; ++iter)
        {
            ...
            for(const auto& other_uav : other_waypoints) {
                if(other_uav.first < uav_id) {
                    double distance2other_waypoint = (candidate - other_uav.second).norm();
...
{
    ros::init(argc, argv, "frontier_exploration_v2");
